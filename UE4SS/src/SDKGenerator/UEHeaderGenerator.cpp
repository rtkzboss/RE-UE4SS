...
namespace RC::UEGenerator
{
    ...
    static auto string_to_lowercase(std::wstring s) -> std::wstring
    {
        std::transform(s.begin(), s.end(), s.begin(), ::towlower);
        return s;
    }
    static auto string_contains_ci(StringViewType haystack, StringViewType needle) -> bool
    {
        return std::search(haystack.cbegin(), haystack.cend(), needle.cbegin(), needle.cend(), [](wchar_t a, wchar_t b) {
                   return std::towlower(a) == std::towlower(b);
               }) != haystack.end();
    }
    static auto string_equals_ci(StringViewType a, StringViewType b) -> bool
    {
        return a.size() == b.size() && std::equal(a.begin(), a.end(), b.begin(), [](auto a, auto b) { return towlower(a) == towlower(b); });
    }
    static auto string_ends_with_ci(StringViewType haystack, StringViewType suffix) -> bool
    {
        return haystack.size() >= suffix.size() && string_equals_ci(haystack.substr(haystack.size() - suffix.size()), suffix);
    }
    static auto string_starts_with_ci(StringViewType haystack, StringViewType prefix) -> bool
    {
        return string_equals_ci(haystack.substr(0, prefix.size()), prefix);
    }
    static auto is_upper(CharType ch) -> bool
    {
        return 'A' <= ch && ch <= 'Z';
    }
    static auto is_lower(CharType ch) -> bool
    {
        return 'a' <= ch && ch <= 'z';
    }
    static auto is_digit(CharType ch) -> bool
    {
        return '0' <= ch && ch <= '9';
    }
    static auto make_display_name(StringViewType string) -> StringType
    {
        StringType out;
        out.reserve(string.size());
        while (string.size() && string[0] == '_')
        {
            string.remove_prefix(1);
        }
        for (auto [i, ch] : string | std::views::enumerate)
        {
            if (i != 0 && (is_upper(ch) && (!is_upper(string[i - 1]) || static_cast<size_t>(i) + 1 < string.size() && is_lower(string[i + 1])) || is_digit(ch) && !is_digit(string[i - 1])))
            {
                out += ' ';
            }
            if (ch == '_')
    ...
    auto UEHeaderGenerator::generate_enum_definition(UEnum* uenum, GeneratedSourceFile& header_data) -> void
    {
        ...
        auto enum_prefix = sanitize_enumeration_name(uenum->GenerateEnumPrefix());
        auto autogenerated_max_name = fmt::format(STR("{}_MAX"), enum_prefix);
        int64_t autogenerated_max_value = highest_enum_value + 1;

        std::optional<int64> last_value;
        std::unordered_set<FName> seen_names;
        for (auto [name, value] : uenum->ForEachName())
        {
            ...
            FlagFormatHelper flags;
            auto name_str = sanitize_enumeration_name(name.ToString());
            if (string_ends_with_ci(name_str, STR("_max")))
            {
                ...
                if (string_equals_ci(name_str, autogenerated_max_name) && value == autogenerated_max_value) continue;
                flags.add_switch(STR("Hidden"));
            }
            else if (string_starts_with_ci(name_str, enum_prefix))
            {
                auto friendly_name = make_display_name(name_str.substr(enum_prefix.size()));
                if (!friendly_name.empty())
                {
                    flags.add_parameter(STR("DisplayName"), friendly_name);
                }
            }

            auto line = std::move(name_str);

            // Taking advantage of GetNameByValue returning the first result for the value to determine if there are any enumerator names that reference an already declared value/name.
            auto first_name_with_value = uenum->GetNameByValue(value);
            if (first_name_with_value != name)
            {
                fmt::format_to(std::back_inserter(line), STR(" = {}"), sanitize_enumeration_name(first_name_with_value.ToString()));
            }
            else if (last_value ? value != *last_value + 1 || last_value == -1 : value != 0)
            {
                if (enum_is_uint8 && value < 0)
                {
                    fmt::format_to(std::back_inserter(line), STR(" = static_cast<uint8>({})"), value);
                }
                else
                {
                    fmt::format_to(std::back_inserter(line), STR(" = {}"), value);
                }
            }

            auto flags_str = flags.build_flag_string();
            if (!flags_str.empty()) fmt::format_to(std::back_inserter(line), STR(" UMETA({})"), flags_str);
            line += STR(",");

            header_data.append_line(line);
            last_value = value;
        }
    ...
    auto UEHeaderGenerator::sanitize_enumeration_name(StringType enum_name) -> StringType
    {
